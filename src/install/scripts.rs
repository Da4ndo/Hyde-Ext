...
pub fn install(choice: &Asset) {
    ...
    println!("{} {}", ":: Installing".blue(), choice.display);
    ...
    let source_path = format!("/path/to/source/{}", choice.name); // Adjust the source path as needed
    let target_path = format!(
        "{}/{}",
        target_dir,
        source_path.split('/').last().unwrap_or_default()
    );
    ...
    if debug_mode {
        println!(
            "{} Attempting to execute the script after copying.",
            ":: Debug:".blue()
        );
    }
